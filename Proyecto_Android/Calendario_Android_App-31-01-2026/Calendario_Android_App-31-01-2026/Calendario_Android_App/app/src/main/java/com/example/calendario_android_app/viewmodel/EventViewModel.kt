...
/**
 * ViewModel principal para la gestión de eventos del calendario.
 * Funcionalidades clave:
 * - Carga y filtrado de eventos por fecha, rango y búsqueda
 * - Gestión de visibilidad de etiquetas y grupos (filtros)
 * - Integración de festivos con eventos de usuario
 * - Creación y eliminación de eventos con permisos
 * - Carga de participantes de eventos grupales
 * Mantiene caché local para optimizar el filtrado sin recargar desde BD.
 */
class EventViewModel : ViewModel() {
    ...
    // Eventos mostrados (después de aplicar filtros)
    private val _eventos = MutableLiveData<List<Evento>>()
    val eventos: LiveData<List<Evento>> = _eventos
    
    // Días del mes que tienen eventos (después de aplicar filtros)
    ...
    // Parámetros del mes actual cargado (para refrescar)
    private var currentMonthLoadParams: Triple<Int, Int, Int>? = null // idUsuario, year, month

    /**
     * Alterna la visibilidad de una etiqueta en los filtros.
     * Reaplica filtros automáticamente.
     */
    ...
    /**
     * Alterna la visibilidad de un grupo completo.
     * Ocultar un grupo oculta todas sus etiquetas y eventos asociados.
     */
    fun toggleGroupVisibility(groupId: Int, isVisible: Boolean) {
        ...
        applyFilters()
        // Actualizar etiquetas para refrescar estado de checks en UI
        _etiquetas.value = _etiquetas.value 
    }
    
    /**
     * Establece una consulta de búsqueda y ejecuta la búsqueda en BD.
     * Los resultados se ordenan por relevancia y proximidad a la fecha actual.
     */
    ...
    /**
     * Aplica todos los filtros activos (grupos, etiquetas, búsqueda) a los datos cacheados.
     * Ordena resultados de búsqueda por relevancia y proximidad temporal.
     */
    private fun applyFilters() {
        ...
        var filteredList = allEventsForList.filter { event ->
            // 1. Verificar grupo (si el evento tiene etiqueta, verificar grupo de la etiqueta)
            if (event.idEtiqueta != null) {
                ...
                // Eventos sin etiqueta se asumen "Personal"
                return@filter false
            }
            
            // 2. Filtro específico de etiqueta
        ...
        // Ordenar resultados de búsqueda si hay consulta activa
        if (searchQuery.isNotBlank()) {
            val today = java.time.LocalDate.now()
            filteredList = filteredList.sortedWith(compareBy(
                // Primario: Relevancia (coincidencia exacta, luego empieza con, luego contiene)
                { event ->
                    when {
                ...
                // Secundario: Proximidad a hoy (diferencia absoluta en días)
                { event ->
    ...
    /**
     * Carga eventos para una fecha específica.
     * Incluye eventos de usuario y festivos.
     */
    fun loadEventosForDate(idUsuario: Int, fecha: LocalDate) {
        viewModelScope.launch {
            ...
            try {
                ...
                // Cargar festivo para esta fecha
    ...
    /**
     * Carga los días del mes que tienen eventos.
     * Carga todos los eventos del mes para permitir filtrado local.
     */
    fun loadDiasConEventos(idUsuario: Int, year: Int, month: Int) {
        currentMonthLoadParams = Triple(idUsuario, year, month)
        viewModelScope.launch {
            try {
                // En lugar de getDiasConEventos, cargamos TODOS los eventos del mes para filtrado local
    ...
    /**
     * Crea un nuevo evento y lo guarda en BD.
     * Registra invitados externos si se proporcionan.
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (idEvento != -1) {
                    ...
                    // Refrescar vista actual (lista)
                    loadEventosForDate(idUsuario, evento.fechaInicio.toLocalDate())
                    // Refrescar puntos si está en el mismo mes
    ...
    /**
     * Carga etiquetas disponibles para el usuario.
     */
    fun loadEtiquetas(idUsuario: Int) {
        viewModelScope.launch {
            try {
                _etiquetas.value = etiquetaDAO.getEtiquetasByUsuario(idUsuario)
                // No resetear hiddenLabelIds para persistir selección al recargar
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    /**
     * Carga grupos del usuario.
     * Crea automáticamente grupo "Personal" si no existe.
     */
    fun loadGrupos(idUsuario: Int) {
        viewModelScope.launch {
            try {
                ...
                if (grupos.isEmpty()) {
                    // Auto-corrección: Crear grupo "Personal" por defecto si falta
                    ...
                    // Deduplicar para evitar múltiples entradas "Personal" en UI si hay duplicados en BD
    ...
    /**
     * Crea una nueva etiqueta y recarga la lista.
     */
    fun createEtiqueta(etiqueta: Etiqueta, idUsuario: Int, idGrupo: Int?) {
        viewModelScope.launch {
            try {
                ...
                if (success) {
                    // Recargar etiquetas para actualizar UI
    ...
    /**
     * Elimina un evento y refresca la vista.
     */
     ...
     * Reglas: Creador (eventos personales) o Admin del grupo (eventos grupales).
     */
    fun checkCanDelete(idUsuario: Int, idEvento: Int, callback: (Boolean) -> Unit) {
        viewModelScope.launch {
            try {
                ...
                // Si no tiene etiqueta, verificar si es el creador
                if (evento.idEtiqueta == null) {
                    callback(evento.idCreador == idUsuario)
                    return@launch
                }

                // Si tiene etiqueta, buscar grupo y verificar admin
                val idGrupo = etiquetaDAO.getGrupoIdByEtiqueta(evento.idEtiqueta)
                if (idGrupo == null) {
                    // Si no hay grupo, es personal (verificar creador)
                    callback(evento.idCreador == idUsuario)
                } else {
    ...
    /**
     * Carga los participantes de un evento grupal.
     * Retorna lista vacía si el evento no tiene grupo asociado.
     */
